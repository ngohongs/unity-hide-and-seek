...
public class EnemyMovementEditor : Editor
{
    ...
    private void OnSceneGUI()
    {
        ...
        if (Hits > 0)
        {
            ...
            {
                Vector3 directionToCollider = (Colliders[i].transform.position - movement.Agent.transform.position).normalized;
                Vector3 directionToPlayer = (movement.Player.transform.position - movement.Agent.transform.position).normalized;
                ...
                //    HitReduction++;
                //}
                else if (Vector3.Dot(directionToPlayer, directionToCollider) > Mathf.Cos(Mathf.Deg2Rad * movement.AngleToPlayer))
                {
                    Handles.color = Color.black;
                    Handles.SphereHandleCap(GUIUtility.GetControlID(FocusType.Passive), Colliders[i].transform.position, Quaternion.identity, 0.25f, EventType.Repaint);
                    Handles.Label(Colliders[i].transform.position, $"{i} same direction as player {Vector3.Dot(directionToPlayer, directionToPlayer)}");
                    Colliders[i] = null;
                    HitReduction++;
                }
            }
            ...
            for (int i = 0; i < Hits; i++)
            {
                if (NavMesh.SamplePosition(Colliders[i].transform.position + (movement.Agent.transform.position - movement.Player.position).normalized * 2, out NavMeshHit hit, 2f, movement.Agent.areaMask))
                {
